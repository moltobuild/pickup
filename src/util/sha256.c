...
/* Two returns rather than a ternary: in C the arms of a `?:` are promoted to
   int, so the whole expression narrows on the way back out however the arms
   are cast. This is the one place a digest comparison folds case, and it is
