...
static const capability catalog[] = {
    /* --- C99 --- */
    {"for_decl", lang_c, "c99", "int main(void){for(int i=0;i<1;i++){(void)i;}return 0;}\n"},
    {"line_comment", lang_c, "c99", "int main(void){// a line comment\nreturn 0;}\n"},
    {"inline_fn", lang_c, "c99",
     "static inline int f(void){return 0;}\nint main(void){return f();}\n"},

    /* --- C11 --- */
    {"static_assert", lang_c, "c11", "int main(void){_Static_assert(1,\"ok\");return 0;}\n"},
    {"generic", lang_c, "c11",
    ...
    {"attr_maybe_unused", lang_c, "c2x", "int main(void){[[maybe_unused]] int x = 0;return 0;}\n"},
    {"typeof", lang_c, "c2x", "int main(void){int a=0;typeof(a) b=a;return b;}\n"},
    {"constexpr", lang_c, "c2x", "int main(void){constexpr int n=0;return n;}\n"},
    {"nullptr", lang_c, "c2x", "int main(void){void *p = nullptr;return p != nullptr;}\n"},
    {"native_bool", lang_c, "c2x", "int main(void){bool ok = true;return ok ? 0 : 1;}\n"},

    /* --- C++11 --- */
    {"auto_type", lang_cxx, "c++11", "int main(){auto x = 0;return x;}\n"},
    {"lambda", lang_cxx, "c++11", "int main(){auto f = [](){return 0;};return f();}\n"},
...
static bool proves(const char *compiler, capability_lang lang, const char *standard,
                   const char *program) {
    ...
    const char *argv[] = {
        compiler,        std_flag,  ARG_LANG, lang == lang_cxx ? LANG_CXX_NAME : LANG_C_NAME,
        ARG_SYNTAX_ONLY, ARG_STDIN, NULL};
    process_result result = process_try(argv, program);
    return result.completed && result.exit_code == 0;
}

bool capability_accepts_standard(const char *compiler, capability_lang lang, const char *standard) {
